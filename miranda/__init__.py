...
class upnp:
    ...
    def initSockets(self, ip, port, iface):
        ...
        try:
            ...
            if self.IFACE != None:
                print(f'\nBinding to interface {self.IFACE.decode()}...\n')
                self.ssock.setsockopt(SOL_SOCKET, SO_BINDTODEVICE, struct.pack(
                    "%ds" % (len(self.IFACE)+1,), self.IFACE))
                self.csock.setsockopt(SOL_SOCKET, SO_BINDTODEVICE, struct.pack(
                    "%ds" % (len(self.IFACE)+1,), self.IFACE))
    ...
    def parseHeader(self, data, header):
        ...
        for line in dataArray:
            # print("[parseHeader] looking at: ",line)
            lowerLine = line.lower()
            # Does this line start with the header we're looking for?
            if lowerLine.startswith(lowerDelim):
                try:
                    res = line.split(':', 1)[1].strip()
                    #print(f"[parseHeader] found {lowerDelim}, returning {res}")
                    return res
                except Exception as e:
                    print(f"Failure parsing header data for {header}. Exception: {e}")
            #else:
                #print(f"[parseHeader] {lowerLine} does not start with {lowerDelim}")
        return defaultRet
    ...
    def parseSSDPInfo(self, data, showUniq, verbose):
        ...
        if messageType != False:
            ...
            if xmlFile == False or host == False or page == False:
                ...
                print(self.STARS)
                print('')
                print(f"xmlFile = {xmlFile}; host = {host}; page = {page}")
                return False
    ...
    def getHostInfo(self, xmlData, xmlHeaders, index):
        ...
        if index >= 0 and index < len(self.ENUM_HOSTS):
            try:
                ...
                self.ENUM_HOSTS[index]['serverType'] = xmlHeaders.get(
                    'Server')
    ...
    def parseDeviceInfo(self, xmlRoot, index):
        ...
        deviceTags = ["friendlyName", "modelDescription", "modelName", "modelNumber",
                      "modelURL", "presentationURL", "UDN", "UPC", "manufacturer", "manufacturerURL"]

        # Find all device entries listed in the XML file
        for device in xmlRoot.getElementsByTagName(devTag):
            try:
                # Get the deviceType string
                deviceTypeName = str(device.getElementsByTagName(
                    deviceType)[0].childNodes[0].data)
            except:
                continue
            ...
            for tag in deviceTags:
                try:
                    deviceEntryPointer[tag] = str(
                        device.getElementsByTagName(tag)[0].childNodes[0].data)
                except Exception as e:
                    if self.VERBOSE:
                        print(
                            'Device', deviceEntryPointer['fullName'], 'does not have a', tag)
                    continue
            ...
            for service in xmlRoot.getElementsByTagName(serviceListTag)[0].getElementsByTagName(serviceTag):
                ...
                for tag in serviceTags:
                    serviceEntryPointer[tag] = str(
                        service.getElementsByTagName(tag)[0].childNodes[0].data)
    ...
    def parseServiceInfo(self, service, index):
        ...
        try:
            ...
            print('Caught exception while parsing Service info for service %s: %s' % (
    ...
    def parseServiceStateVars(self, xmlRoot, servicePointer):
        ...
        try:
            stateVars = xmlRoot.getElementsByTagName(serviceStateTable)[
                0].getElementsByTagName(stateVariable)
        except:
        ...
        for var in stateVars:
            for tag in varVals:
                ...
                try:
                    ...
                    print('Failed to get service state variable name for service %s!' %
                          servicePointer['fullName'])
                    continue
                ...
                try:
                    vals = var.getElementsByTagName(allowedValueList)[
                        0].getElementsByTagName(allowedValue)
                except:
                    pass
                else:
    ...
    def updateCmdCompleter(self, struct):
        ...
        try:
            ...
            print(
                f"Error ({e}) updating command completer structure; some command completion features might not work...")
        return
...
def msearch(argc, argv, hp):
    ...
    st = "schemas-upnp-org"
    myip = get_address_of_interface(hp.IFACE)
    lport = hp.port
    ...
    if argc >= 3:
        if argc == 4:
            ...
            searchName = argv[2]
        st = "urn:%s:%s:%s:%s" % (
            st, searchType, searchName, hp.UPNP_VERSION.split('.')[0])
    else:
        st = defaultST
    ...
    print(f"Entering discovery mode for '{st}', Ctrl+C to stop...")
    print('')
    ...
    if server == False:
        ...
        return
    print(f"Listening on {myip}:{lport}")
    ...
    while True:
        try:
            ...
            print(f'\nDiscover mode halted with exception {e}...')
            break
...
def pcap(argc, argv, hp):
    ...
    while True:
        try:
            ...
            print(f"\nPassive mode halted with exception ({e})...")
            break
        ...
        if action == 'list':
            ...
            if argc == 3:
                ...
                try:
                    ...
                    if hostInfo['dataComplete'] == True:
                        ...
                        print(
                            "Can't show host info because I don't have it. Please run 'host get %d'" % index)
                        ...
                        if hostInfo != False:
                            ...
                            if hostInfo['dataComplete'] == False:
                                ...
                                if hp.getHostInfo(xmlData, xmlHeaders, index) == False:
                                    print(
                                        "Failed to get device/service info for %s..." % hostInfo['name'])
                                    return
            ...
            if argc != 6:
                ...
                try:
                    ...
                    print(
                        "Are you sure you've run 'host get %d' and specified the correct service name?" % index)
                    return False
                ...
                for argName, argVals in actionArgs.items():
                    ...
                    if argVals['direction'].lower() == 'in':
                        ...
                        try:
                            ...
                            sendArgs[argName] = (
                                uInput.strip(), stateVar['dataType'])
                        except KeyboardInterrupt:
                            print("")
                            return
                        print('')
                ...
                soapResponse = hp.sendSOAP(
                    hostInfo['name'], fullServiceName, controlURL, actionName, sendArgs)
    ...
    if saveType == 'struct':
        ...
        try:
            ...
            print("Host info for '%s' saved to '%s'" %
                  (hp.ENUM_HOSTS[index]['name'], fileName))
        except Exception as e:
            ...
            if opt == '-s':
                ...
                try:
                    if platform.system() != 'Windows':
                        ...
                        networkInterfaces.append(
                            'Run ipconfig to get a list of available network interfaces!')
                except Exception as e:
